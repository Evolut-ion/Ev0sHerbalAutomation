...
public class WoodCutterComponentSystem extends EntityTickingSystem<ChunkStore> {
    @Nonnull
    ...
    private final ComponentType<ChunkStore, WoodCutter> componentType;

    public WoodCutterComponentSystem(ComponentType<ChunkStore, WoodCutter> componentType) {
    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk,
        ...
        if (debugCounter++ % 300 == 0) {
            System.out.println("[WoodCutterSystem] tick called, index=" + index + " componentType=" + componentType);
        }
        if (componentType == null) return;
        WoodCutter wc = archetypeChunk.getComponent(index, componentType);
        if (debugCounter % 300 == 1) {
            System.out.println("[WoodCutterSystem] component lookup: " + (wc != null ? "FOUND" : "NULL"));
        }
        if (wc != null) {
            wc.tick(dt, index, archetypeChunk, store, commandBuffer);
        }
    }
